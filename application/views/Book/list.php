...
<script type="text/javascript">
  ...
  //       });
		...
		$.get("book/search_book_list", option,function( aData ){
			...
			if ( Object.keys(aData).length > 1) {
				...
				switch (v.status) {
					...
					str_html += (v.status == "wait_payment") ? " <td align='center'><i class='fa fa-credit-card' style='font-size:20px' onclick='open_form_payment("+v.id+")'></i></td>" : "<td></td>"; 
	...
	function open_form_payment( book_id ){
		$.ajax({
            url:  "book/get_form_payment",
            type: 'POST',
            dataType: 'json',
            success: function (response) {
